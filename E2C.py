...
class VAE(nn.Module):
    def __init__(self, bottleneck_dim=100, f=(3, 16)):
        super(VAE, self).__init__()
        self.bottleneck_dim = bottleneck_dim
        self.encoder = nnetD(1, nz=1024, nc=1, lf=f)
...
if __name__ == '__main__' : 
    ...
    encoder = nnetD(1, nz=1024, nc=1, lf=hid_filter)
    decoder = nnetG(1, nz=bottleneck_dim, nc=1, ff=hid_filter)
    transnet = transNet(z_dim=bottleneck_dim, h_dim=2*bottleneck_dim, bs=batch_size)

    encoder.cuda(); decoder.cuda(); transnet.cuda()
    ...
    def train(epoch):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            lambda_kl = 0 if epoch == 0 else 5e-6
